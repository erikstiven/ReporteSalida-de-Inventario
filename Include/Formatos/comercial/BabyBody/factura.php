    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                unset($nombre_color);
                $nombre_color = array_dato($oIfC, $sql_color, 'estado', 'color');

                $sql_prod = "SELECT SUBSTRING(prod_cod_prod, 1, 3)as cod_prod, prod_det_prod from saeprod where prod_cod_prod='$dfac_cod_prod'";
                //echo $sql;exit;
                $dfac_nom_prod = consulta_string_func($sql_prod, 'prod_det_prod', $oIfC, '');
                $cod_prod = consulta_string_func($sql_prod, 'cod_prod', $oIfC, '');
                $dfac_det_dfac = $dfac_nom_prod;
                ...
                if($para_dfa_para==1){
                    ...
                    if($ctralter != 0){
                        $deta .= ' <td style="width: 38%;">' . $nombre_completo . '</td>';
                      }
                    else{
                            $deta .= ' <td style="width: 50%;">' . $nombre_completo . '</td>';  
                        }
                ...
                if($para_dfa_para==0){
                    if($ctralter != 0){
                        $deta .= ' <td style="width: 18%;">assasas</td>';

                    }
                    else{
                        $deta .= ' <td style="width: 30%;">12121</td>';  
                    }
              
                    $deta .= ' <td style="width: 20%;">asdasdas</td>';  
