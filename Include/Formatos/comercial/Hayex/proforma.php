...
include_once(path(DIR_INCLUDE) . 'comun.lib.php');
    ...
    if ($oIfx->Query($sqlPed)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $cliente .= ' <td colspan="3">' . htmlentities($direccion) . '</td>';
                $cliente .= ' </tr>';
                ...
                $cliente .= ' <b><td style="width: 14% "></td></b>';
                $cliente .= ' <td style="width: 26% "></td>';
                $cliente .= ' </tr>';
    ...
    $deta .= ' <tr>';
	//$deta .= ' <b> <th class="diagrama" style="width: 15%;" align="center">UBICACION</th> </b>';
    $deta .= ' <b> <th class="diagrama" style="width: 5%;" align="center">N°</th> </b>';
    $deta .= ' <b> <th class="diagrama" style="width: 19%;" align="center">CODIGO</th> </b>';
    $deta .= ' <b> <th class="diagrama" style="width: 40%;" align="center">DESCRIPCION</th> </b>';
	...
	//$deta .= ' <b> <th class="diagrama" style="width: 10%;" align="center">DESPACHADO</th> </b>';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
				...
				$sql = "select prod_cod_barra,  prod_cod_marc , marc_cod_marc, marc_des_marc
				...
				if(empty($prod_cod_barra)){
					$sql = "select prcb_cod_barr  from saeprcb where
								prcb_cod_empr = $idEmpresa and
								prcb_cod_sucu = $idSucursal and
								prcb_cod_prod = '$dfac_cod_prod' ";
					$prod_cod_barra = consulta_string_func($sql, 'prcb_cod_barr', $oIfxA, '');
				}
				
                $deta .= ' <tr>';
				//$deta .= ' <td style="width: 15%;">'.$ubicacion.'</td>';
                $deta .= ' <td style="width: 5%;" align="center">' . $i . '</td>';
                $deta .= ' <td style="width: 19%;" align="center">' . $dfac_cod_prod . '</td>';
                $deta .= ' <td style="width: 40%;">' . htmlentities($dfac_nom_prod) . '</td>';
                ...
                //$deta .= ' <td style="width: 10%;" align="right"></td>';		
    ...
    $adicional .= ' <table style="margin-top:50px; width: 100%; font-size: 14px; border: black 1px solid ; margin-top:30px;" cellspacing="0" >';
    
    $adicional .= '<tr>';
    $adicional .= '<td style="width: 100%;background: red; color:white;" align="center"><b>INFORMACIÓN BANCARIA ' . $razonSocial . ' </b></td>';
    $adicional .= '</tr>';

    $adicional .= '<tr>';
    $adicional .= '<td style="width: 100%;" >Cuenta Corriente Banco Pichincha Nro.  2100298610, ' . $razonSocial . ', RUC '.$ruc_empr.'</td>';
    $adicional .= '</tr>';

    $adicional .= '<tr>';
    $adicional .= '<td style="width: 100%;">&nbsp;</td>';
    $adicional .= '</tr>';


    $adicional .= '<tr>';
    $adicional .= '<td style="width: 100%;background: red; color:white;" align="center"><b>OBSERVACIONES </b></td>';
    $adicional .= '</tr>';

    $adicional .= '<tr>';
    $adicional .= '<td style="width: 100%;">No incluye costo o valor por flete dentro o fuera de la ciudad de Quito.<br>
    Precios y stock pueden variar sin previo aviso.<br>
    Una vez salida la mercadería, la empresa no se responsabiliza por daños, hurtos que sufra la mercadería.<br>
    Este documento no se puede distribuir sin autorización de '.$razonSocial.'.<br>
    La información que contiene este documento es de carácter confidencial.</td>';
    $adicional .= '</tr>';

    $adicional .= '<tr>';
    $adicional .= '<td  align="center" style="width: 100%;"><br>'.$banner.'</td>';
    $adicional .= '</tr>';
