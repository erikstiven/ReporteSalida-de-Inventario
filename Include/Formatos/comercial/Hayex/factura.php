...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal = '', &$rutaPdf = '')
{
    ...
    $deta = '<br><br> <table style="width: 100%; font-size: 12px; border-radius: 5px; border-collapse: collapse;" border="1">';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $deta .= ' <tr>';
                $deta .= ' <td style="width: 15%; font-size: 10px;">' . $dfac_cod_prod . '</td>';
                $deta .= ' <td style="width: 34%; font-size: 10px;">' . $dfac_nom_prod . '</td>';
                $deta .= ' <td style="width: 10%; font-size: 10px;" align="right">' . number_format($dfac_cant_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 15%; font-size: 10px;" align="right">' . number_format($dfac_precio_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 11%; font-size: 10px;" align="right">' . round($dfac_des1_dfac, 0) . '</td>';
                $deta .= ' <td style="width: 15%; font-size: 10px;" align="right">' . number_format($dfac_mont_total, 2, '.', ',') . '</td>';
                $deta .= ' </tr>';
            } while ($oIfx->SiguienteRegistro());
        }
    }
    ...
    $totales .= ' <tr>';
    $totales .= ' <b> <td style="width: 25%;">SUBTOTAL ' . round($porcIva) . '%:</td> </b>';
    $totales .= ' <td style="width: 10%;" align="right">' . number_format($fact_con_miva, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

    $totales .= ' <tr>';
    $totales .= ' <b> <td style="width: 25%;">SUBTOTAL 0%:</td> </b>';
    $totales .= ' <td style="width: 10%;" align="right">' . number_format($fact_sin_miva, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';
    ...
    $legend = '<page_footer>
        <div style="font-size: 12px; color: #6B6565; background-color: transparent;" align="center">
        Este comprobante electronico ha sido generado a traves de Sisconti S.A. - Facturacion Electronica<br>www.sisconti.com
        </div>
    </page_footer>';
