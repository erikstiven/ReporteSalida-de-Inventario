...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal = '', &$rutaPdf = '')
{
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $sql_prod = "SELECT  prod_cod_barra from saeprod where prod_cod_prod='$dfac_cod_prod' and prod_cod_empr=$idEmpresa";
                $prod_cod_alt = trim(consulta_string_func($sql_prod, 'prod_cod_barra', $oIfxA, ''));
                ...
                $deta .= ' <td style="width: 28%;">' . $dfac_det_dfac . '</td>';
                //$deta .= ' <td style="width: 6%;" align="center">' . $array_unidad[$dfac_cod_unid] . '</td>';
                $deta .= ' <td style="width: 18%;"></td>';
    ...
    $totales .= ' <tr>';
    $totales .= ' <td style="width:100%;" ><b>Información Adicional</b><br>' . $fact_cm1_fact . '</td>';
    $totales .= ' </tr>';
    $totales .= ' </table>';
