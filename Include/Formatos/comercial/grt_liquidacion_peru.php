...
function grt_liquidacion_peru($total,$id_empl = '', $id_empresa = 0, $id_rubr = 'RGRTD', $id_anio = 0, &$rutaPdf = '')
{
    //Definiciones
    global $DSN_Ifx;
    ...
    $nombre_archivo = 'GRT_LIQUIDACION_' . $id_anio . '_' . $id_rubr . '_' . $id_empl;
    ...
    $rgrt = grt_aplicable($id_anio,$id_rubr, $id_empresa, $oIfx);
    $rgrt_est_trun = es_trunca($rgrt['rgrt_fec_inic'],$rgrt['rgrt_fec_fina'],$id_empl,$id_empresa,$oIfx);

    if($rgrt_est_trun != false){
        $rgrt['rgrt_fec_fina'] = $rgrt_est_trun;
    }

    // Paso 2: determinar los valores variables aplicables

    $rgrt_val_hhee = promedio_horas_extras($rgrt['rgrt_per_inic'], $rgrt['rgrt_per_fina'], $rgrt['rgrt_fec_fina'], $id_empl, $id_empresa, $oIfx);
    $rgrt_val_comi = promedio_comisiones($rgrt['rgrt_per_inic'], $rgrt['rgrt_per_fina'], $rgrt['rgrt_fec_fina'], $id_empl, $id_empresa, $oIfx);


    $rgrt_dia_trab = calcular_intervalo_trabajado($rgrt['rgrt_per_inic'], $rgrt['rgrt_fec_fina'], $id_empl, $id_empresa, $oIfx);

    $sql = "SELECT pago_val_pago FROM saepago WHERE pago_cod_rubr= 'RASFM' AND pago_cod_empl = '{$id_empl}' and pago_cod_empr = '{$id_empresa}' AND pago_per_pago = '{$rgrt['rgrt_per_fina']}'";
    ...
    $remuneracion = [
        ...
        "Promedio de HHEE" => $rgrt_val_hhee,
        "Promedio de Comisiones" => $rgrt_val_comi
    ];
    ...
    $documento='
        <div style="margin-left: 64px; margin-right: 32px;">
            ...
            <br/>
            <h1>LIQUIDACIÓN DEL DEPOSITO SEMESTRAL DE GRATIFICACIONES</h1>
            <br/>
            <table cellspacing="4"> 
                <tr>
                ...
                <tr>
                    <td colspan="6" align="center">Del '. $rgrt['rgrt_fec_inic']->format('d/m/Y') .' al '. $rgrt['rgrt_fec_fina']->format('d/m/Y') .'</td>
                </tr>
                <tr>
                    <td colspan="6">La presente constancia del depósito de gratificación realizado el '. (new DateTime())->format('d/m/Y') .' por los siguientes montos:</td>
                </tr>
            </table>
            <table cellspacing="4"> 
                <tr>
...
function grt_aplicable($rgrt_ani_fina, $rgrt_cod_rubr, $empr_cod, $oIfx){
    $sql = "SELECT 
                pnom_cod_rubr,
                pnom_cod_pnom,
                pnom_mes_inic,
    ...
    if($pnom_mes_fina < $pnom_mes_inic){
        $rgrt_ani_inic = ((int)$rgrt_ani_fina) -1;
        $pnom_per_inic = $rgrt_ani_inic.str_pad($pnom_mes_inic, 2, '0', STR_PAD_LEFT);
        $pnom_per_fina = $rgrt_ani_fina.str_pad($pnom_mes_fina, 2, '0', STR_PAD_LEFT);
    }else{
        $rgrt_ani_inic = $rgrt_ani_fina;
        $pnom_per_inic = $rgrt_ani_inic.str_pad($pnom_mes_inic, 2, '0', STR_PAD_LEFT);
        $pnom_per_fina = $rgrt_ani_fina.str_pad($pnom_mes_fina, 2, '0', STR_PAD_LEFT);
    }
    $rgrt_fec_fina = new DateTime($rgrt_ani_fina.'-'.str_pad($pnom_mes_fina, 2, '0', STR_PAD_LEFT).'-'.str_pad($pnom_dia_fina, 2, '0', STR_PAD_LEFT));
    $rgrt_fec_inic = new DateTime($rgrt_ani_inic.'-'.str_pad($pnom_mes_inic, 2, '0', STR_PAD_LEFT).'-'.str_pad($pnom_dia_inic, 2, '0', STR_PAD_LEFT));
    ...
    $result = [
        ...
        'rgrt_fec_fina' => $rgrt_fec_fina,
        'rgrt_fec_inic' => $rgrt_fec_inic
    ];
    $oIfx->Free();
    return $result;
}
...
function calcular_remuneracion_precalculada($remuneracion,$rgrt,$empr_cod,$cod_empl,$oIfx){
    $sql = "SELECT
            ...
            WHERE
                ...
                pemp_per_pemp BETWEEN '{$rgrt["rgrt_per_inic"]}' AND '{$rgrt["rgrt_per_fina"]}'
            ;";
    $oIfx->Query($sql);
