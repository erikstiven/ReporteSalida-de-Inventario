...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
  ...
  $arrayTipDoc = array_dato($oCon, $sql, 'clpv_cod_clpv', 'clv_con_clpv');
        ...
        if ($oIfx->NumFilas() > 0) {
            do {
               ...
               // $sql = "select tcmp_cod_tcmp, tcmp_des_tcmp from saetcmp where tcmp_cod_tcmp = '$fact_tip_vent';";
                ...
                //CABECERA FACTURA DATOS DE LA EMPRESA
                ...
                $logo .= '<tr>';
                $logo .= '<td  width="335">'.$logo_empresa.'</td>';
                $logo .= '</tr>';
                $logo .= '<tr>';
                $logo .= '<td width="335" style="font-size:15px;"><b>' .$razonSocial . '</b></td>';
                $logo .= '</tr>';
                $logo.='</table>';
                $logo .= '</td>';
                ...
                $logo .= '<table  style="margin: 0px;">';
                $logo .= '<tr>';
                $logo .= '<td  width="260" style="font-size:18px;" colspan="2" align="right"><b>'.$titulo.'</b><br>Con Derecho a Crédito Fiscal</td>';
                $logo .= '</tr>';
                $logo .= '<tr>';
                $logo .= '<td width="130" style="font-size:15px;" align="right"><b>NIT</b></td>';
                $logo .= '<td width="130" style="font-size:15px;" align="right">' . $ruc_empr . '</td>';
                $logo .= '</tr>';
                $logo .= '<tr>';
                $logo .= '<td width="130" style="font-size:15px;" align="right"><b>'.$titulo.' N°</b></td>';
                $logo .= '<td width="130" style="font-size:15px;" align="right">' . $fact_num_preimp . '</td>';
                $logo .= '</tr>';
                $logo .= '<tr>';
                $logo .= '<td width="130" style="font-size:15px;" align="right"><b>Código de autorización</b></td>';
                $logo .= '<td width="130" style="font-size:15px;" align="right">'.$fact_auto_sri.'</td>';
                $logo .= '</tr>';
                $logo.='</table>';
                $logo .= '</td>';
                ...
                $logo .= '<tr>';
                $logo .= '<td  colspan="2"><b>Casa Matriz</b><br>'.$dirMatriz.'<br>Telf. '.$fact_tlf_cliente.'<br>'.$ciudad.', '.$pais.'</td>';
                $logo .= '</tr>';

                $logo.='</table>';

                //DATOS DEL CLIENTE
                $logo .='<table border="0"  style="width: 100%; margin-top:25px;"  cellspacing="0">';
                $logo .= '<tr>';
                $logo .= '<td ><b>Lugar y fecha:</b>&nbsp;&nbsp;'.$ciudad.', '.$fact_fech_fact.'</td>';
                $logo .= '</tr>';
                $logo .= '<tr>';
                $logo .= '<td ><b>Nombre/Razón Social:</b>&nbsp;&nbsp;'.$fact_nom_cliente.'</td>';
                $logo .= '</tr>';
                $logo .= '<tr>';
                $logo .= '<td ><b>NIT/CI/CEX:</b>&nbsp;&nbsp;'.$fact_ruc_clie.'</td>';
                $logo .= '</tr>';
                $logo .= '<tr>';
                $logo .= '<td ><b>Código del cliente:</b>&nbsp;&nbsp;'.$fact_ruc_clie.'</td>';
                $logo .= '</tr>';
                $logo.='</table>';
    ...
    $deta .= ' <table style="width: 90%;  font-size: 12px;   margin-top:25px;" cellspacing=0>';
    $deta .= ' <tr>';
    $deta .= ' <b> <td style="border-top: gray 1.5px solid;   width: 20%; font-size:13px;" align="left" height="30">Código Producto/<br>Servicio</td> </b>';
    $deta .= ' <b> <td style="border-top: gray 1.5px solid;   width: 32%; font-size:13px;" align="left" height="30">Descripción</td> </b>';
    $deta .= ' <b> <td style="border-top: gray 1.5px solid;   width: 10%; font-size:13px;" align="left" height="30">Unidad medida</td> </b>';
    $deta .= ' <b> <td style="border-top: gray 1.5px solid;   width: 12%; font-size:13px;" align="left" height="30">Cantidad</td> </b>';
    $deta .= ' <b> <td style="border-top: gray 1.5px solid;   width: 12%; font-size:13px;" align="right" height="30">Precio<br>unitario</td> </b>';
    $deta .= ' <b> <td style="border-top: gray 1.5px solid;   width: 12%; font-size:13px;c" height="30" align="right">Descuento</td> </b>';
    $deta .= ' <b> <td style="border-top: gray 1.5px solid;   width: 12%; font-size:13px;" align="right" height="30">Subtotal</td> </b>';
    $deta .= ' </tr>';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $sqlu="select unid_sigl_unid, unid_nom_unid from saeunid where unid_cod_unid=$dfac_cod_unid";
                $unidad= consulta_string($sqlu,'unid_nom_unid', $oCon,'');
                }
                else{
                    $unidad='';
                }
                ...
                $deta .= ' <tr>';
                $deta .= ' <td style="width: 20%;">'.$dfac_cod_prod.'</td>';
                $deta .= ' <td style="width: 32%;">'.$dfac_det_dfac.'</td>';
                $deta .= ' <td style="width: 10%;" align="left" >' . $unidad . '</td>';
                $deta .= ' <td style="width: 12%;" align="center" >' . number_format($dfac_cant_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 12%;" align="right" >' . number_format($dfac_precio_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 12%;" align="right" >' . number_format($porcentaje_descuento, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 12%;" align="right" >' . number_format($dfac_mont_total, 2, '.', ',') . '</td>';
                $deta .= ' </tr>';
                $ctrl++;
            }while ($oIfx->SiguienteRegistro());
                $deta .= ' <tr>';
                $deta .= ' <td style="width: 20%; border-top: light gray 0.7px solid" ></td>';
                $deta .= ' <td style="width: 32%; border-top: light gray 0.7px solid"></td>';
                $deta .= ' <td style="width: 10%; border-top: light gray 0.7px solid;"></td>';
                $deta .= ' <td style="width: 12%; border-top: light gray 0.7px solid;"></td>';
                $deta .= ' <td style="width: 12%; border-top: light gray 0.7px solid;"></td>';
                $deta .= ' <td style="width: 12%; border-top: light gray 0.7px solid;"></td>';
                $deta .= ' <td style="width: 12%; border-top: light gray 0.7px solid;"></td>';
                $deta .= ' </tr>';
    ...
    $totales ='<table style="width: 90%;  font-size: 12px; margin-top: 5px;" cellspacing="0"  align="left">';
    $totales .='<tr>
    ...
    <tr>
    <td style="background: #e9f0f8;" width="390" height="110"><div style="margin-left:10px"><b><br><br>Términos de pago Pago:</b><br><br>'. $fpagop_des_fpagop.'</div></td>
    </tr>';
    ...
    $totales .= ' <table style=" font-size: 12px;margin-left: 45px;"   cellspacing="0" align="left">';

    $totales .= ' <tr>';
    $totales .= ' <b> <td width="160" height="18" >Subtotal:</td> </b>';
    $totales .= ' <td  width="96" align="right">' . number_format($fact_tot_fact, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

   

    $totales .= ' <tr>';
    $totales .= ' <b> <td width="160" height="18" >Descuento adicional:</td> </b>';
    $totales .= ' <td width="96" align="right">' . number_format($fact_dsg_valo, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';


    $totales .= ' <tr>';
    $totales .= ' <b> <td width="160" height="18" >Total Valor</td> </b>';
    $totales .= ' <td width="96" align="right">' . number_format($fact_tot_fact, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

    //VALIDAR DE DONDE TOMAR VALOR

    $totales .= ' <tr>';
    $totales .= ' <b> <td width="160" height="18">Importe base Crédito Fiscal:</td> </b>';
    $totales .= ' <td width="96" align="right">' . number_format($fact_tot_fact, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

   
    $totales .= ' <tr >';
    $totales .= ' <b> <td width="160"  height="25" style="background:#c6c9d0; font-size:15px;" valign="middle">&nbsp;&nbsp;Monto a pagar '.$eti_mone.'</td> </b>';
    
    $totales .= ' <td width="96"   style="border: #c6c9d0 0.7px solid;font-size:15px;" align="right"><b>' . number_format($fact_tot_fact, 2, '.', ',') . '</b></td>';
    $totales .= ' </tr>';
    $totales .= ' </table>';
    ...
    $tableLeyenda ='<table border="0"  style="width: 100%;font-size: 11px;margin-top:25px;" cellspacing="0">';
    $tableLeyenda .= '<tr>';


    $tableLeyenda .= '
    <td valign="top" width="45%">';
    $tableLeyenda .= '<table style="  font-size: 11px; margin-top:10px;" cellspacing="0" align="left">';
    ...
    $tableLeyenda .= '<tr>';
    $tableLeyenda .= '<td   width="380"><img src="'.$ruta.'"></td>';
    ...
    $tableLeyenda .= '<td valign="top" width="55%">';
    $tableLeyenda .= '<table  height="300" style="font-size: 13px;cellspacing="0" >';
    
    $tableLeyenda .= '<tr>';

    $tableLeyenda .= '<td width="500" height="88">
    <b>Son: '.$con_letra.'</b></td>';
    $tableLeyenda .= '</tr>';
    ...
    $tableLeyenda .='<table border="0"  style="width: 100%;font-size: 10px;margin-top:25px;" cellspacing="0">';
    $tableLeyenda .= '<tr>';
    $tableLeyenda .='<td style="font-size: 12px; color: #6B6565; background-color: transparent;" ><b>Esta factura contribuye al desarrollo del país, el uso ilícito será sancionado penalmente de acuerdo a Ley.</b><br><br>
    Ley N° 453: El proveedor debe exhibir certificaciones de habilitación o documentos que acrediten las capacidades u ofertas de servicios
    especializados.<br>
    “Este documento es la Representación Gráfica de un Documento Fiscal Digital emitido en una modalidad de facturación en línea”</td>';
    $tableLeyenda .= '</tr>';
    $tableLeyenda .= '</table>';
