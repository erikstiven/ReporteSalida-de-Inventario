...
//INFORMACION DE LA EMPRESA
 ...
 $sql = "select empr_cod_pais,empr_cm1_empr, empr_rimp_sn, empr_nom_empr, empr_ruc_empr , empr_dir_empr, empr_conta_sn, empr_num_resu, empr_path_logo, empr_iva_empr,empr_tel_resp, empr_ac1_empr, empr_ac2_empr, empr_mai_empr, empr_tip_empr
    ...
    $sqlm="select minv_fmov,minv_tip_ord, minv_num_sec, minv_cod_clpv, minv_cod_mone,minv_cm1_minv,minv_tot_minv, minv_iva_valo, minv_dge_valo,
    ...
    $fpago=consulta_string($sqlm, 'minv_cod_fpagop', $oIfx, '');
    ...
    $minv_dge_valo = consulta_string($sqlm, 'minv_dge_valo', $oIfx, '');
    ...
    $sql="select mone_des_mone from saemone where mone_cod_mone=$mone_cod";
    ...
    $tipo_cta = consulta_string($sql, 'clpv_tip_ctab', $oIfx, '');
    ...
    $ciud_clpv= consulta_string($sql, 'clpv_cod_ciud', $oIfx, '');
    $ruc_ctab =  trim(consulta_string($sql, 'clpv_ruc_tran', $oIfx, ''));
    ...
    $deta .= ' <b> <td style="border-left: black 1px solid; border-bottom: black 1px solid; width:  10%; font-size:12px;" align="center" height="20">VALOR DE SERVICIO C/U</td> </b>';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $deta .= ' <td align="right"  style="width: 12%; border-bottom: black 1px solid; border-left: black 1px solid;">' . number_format($costo, 2, '.', ',') . '</td>';
                $deta .= ' <td align="right"  style="width: 10%; border-bottom: black 1px solid; border-left: black 1px solid;">' . number_format($cantidad, 2, '.', ',') . '</td>';
    ...
    $totales .= ' <tr>';
    $totales .= ' <b> <td width="100" height="18" >SUB TOTAL</td> </b>';
    ...
    $totales .= ' <tr>';
    $totales .= ' <b> <td width="100" height="18" >'.$array_imp ['IVA'].' '.$array_porc ['IVA'].'%</td> </b>';
    $totales .= ' <td width="115" align="right">' . number_format( $minv_iva_valo, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';


    $totales .= ' <tr>';
    $totales .= ' <b> <td width="100" height="18" >TOTAL A PAGAR</td> </b>';
    ...
    $firmas = '<table style="width:100%; margin-top: 65px" align="center" >
        <tr>
            ...
            <td style="width:21.25%; font-size:10px; text-align: center;border-top : 2px solid black;"><b>LOGISTICA</b></td>
            <td style="width:5%;"></td>
