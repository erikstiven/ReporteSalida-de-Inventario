...
function reporte_factura_personalizado_externo($id = '', $nombre_archivo = '', $idEmpresa ='',$idSucursal ='', &$rutaPdf = '') {
    global $DSN_Ifx, $DSN;
    ...
    if ($oIfx->Query($sqlFac)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $moneda_local = consulta_string($sql_moneda, 'pcon_mon_base', $oCon, '');
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $deta .= ' <tr>';
                $deta .= ' <td style="width: 55%;">'.$dfac_nom_prod.' '.$dfac_det_dfac.'</td>';
...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
  ...
  $arrayTipDoc = array_dato($oCon, $sql, 'clpv_cod_clpv', 'clv_con_clpv');
    ...
    if($tipo_pdf=='F'&&!empty($fact_cod_detra)){

        $totales .= '<tr>';
        $totales .= '<td style=" border-top: '.$empr_web_color.' 0.5px solid;" ></td>';
        $totales .= '</tr>';
        $totales .= '<tr>';
        $totales .= '<td width="390" height="56" ><b>CUENTA DE DETRACCIÓN: '.$num_cuenta_detra.'</b>
        </td>';
        $totales .= '</tr>';
...
$ruta_dir = DIR_FACTELEC . 'modulos/envio_documentos/qr_guias';
    if (!file_exists($ruta_dir)){
        mkdir($ruta_dir,0777,true);
    }
    
file_put_contents(DIR_FACTELEC . 'modulos/envio_documentos/qr_guias/GUIA_'.$idgui.'.png', $imageData); // Guardamos el resultado

$ruta=DIR_FACTELEC . 'modulos/envio_documentos/qr_guias/GUIA_'.$idgui.'.png';
...
$documento .= '</page>';
    ...
    $ruta = DIR_FACTELEC . 'modulos/envio_documentos/upload/pdf/gui_' . $nombre_archivo . '.pdf';
    $html2pdf->Output($ruta,'F');
    $rutaPdf = $ruta;
