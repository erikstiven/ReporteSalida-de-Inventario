...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
  ...
  $arrayTipDoc = array_dato($oCon, $sql, 'clpv_cod_clpv', 'clv_con_clpv');
    ...
    if(!empty($fact_cod_detra)){
        ...
        $sql="select tret_porct, tret_mont_min from saetret where tret_cod='$fact_cod_detra'";
        ...
        if(round($fact_tot_fact,2)>round($tret_mon_min,2)){
            ...
            $valor_neto=$fact_tot_fact-$valdetra;

            $totales .='<tr>
            <td width="390"><b>Detracción al '.round($fact_porc_detra,2).'%:</b> S/ '.number_format($valdetra, 2, '.', ',').' </td>
            </tr>
            <tr>
            <td width="390"><b>Neto a Pagar:</b> S/ '.number_format($valor_neto, 2, '.', ',').'</td>
            </tr>';
        }
    ...
    /*if($tipo_pdf=='F'&&$idEmpresa==1){

        $totales .= '<tr>';
        $totales .= '<td style=" border-top: '.$empr_web_color.' 0.5px solid;" ></td>';
        $totales .= '</tr>';
        $totales .= '<tr>';
        $totales .= '<td width="390" height="56" ><b>CUENTA DE DETRACCIÓN: 00-381-284875</b><br>
        Código BB Y SS sujetos a DETRACCIÓN: <b>019 y 017</b><br>
...
function reporte_notaCredito_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
 ...
 $empr_cod_pais = round(consulta_string($sql, 'empr_cod_pais', $oIfx, 0));
    ...
    if ($oIfx->Query($sqlFac)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $sql = "select mone_des_mone from saemone where mone_cod_mone =  $ncre_cod_mone;";
                $moneda= consulta_string($sql,'mone_des_mone', $oCon,'');
                ...
                $logo .= '<td align="left"  width="400">';
                $logo .= '<table  style="margin: 0px;" >';
                $logo .= '<tr>';
                $logo .= '<td align="center">'.$logo_empresa.'</td>';
                $logo .= '<td width="283" style="font-size:14px;"><b>' . $razonSocial . '</b><br>'.$dirMatriz.'<br></td>';
                $logo .= '</tr>';
                $logo.='</table>';
                $logo .= '</td>';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $deta .= ' <td style="width: 12%;" align="right"  style="border-left: '.$empr_web_color.' 0.5px solid;">' . number_format($dncr_cant_dfac, 4, '.', ',') . '</td>';
                $deta .= ' <td style="width: 10%;" align="right"  style="border-left: '.$empr_web_color.' 0.5px solid;">' . number_format($dncr_precio_dfac, 4, '.', ',') . '</td>';
                $deta .= ' <td style="width: 10%;" align="right"  style="border-left: '.$empr_web_color.' 0.5px solid;">' . number_format($dncr_mont_total, 4, '.', ',') . '</td>';
                $deta .= ' </tr>';
                $ctrl++;
    ...
    $total = number_format($ncre_iva + $ncre_con_miva + $ncre_sin_iva, 2, '.', '');
    $V = new EnLetras();
    $con_letra = strtoupper($V->ValorEnLetrasMone($total, $moneda));
    ...
    <tr>
    <td width="440" height="162" >SON: '.$con_letra.'</td>
    </tr>
    </table>
    </td>
    ...
    $totales .= ' <tr>';
    $totales .= ' <b> <td width="140" height="18" align="right">Total Op. Gravadas S/</td> </b>';
    $totales .= ' <td  width="100" align="right">' . number_format($tot_opgrav, 2) . '</td>';
    $totales .= ' </tr>';

   

    $totales .= ' <tr>';
    $totales .= ' <b> <td width="140" height="18" align="right">Total Op. Inafectas S/</td> </b>';
    $totales .= ' <td width="110" align="right">' . number_format( $tot_opinafe, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';


    $totales .= ' <tr>';
    $totales .= ' <b> <td width="140" height="18"align="right">Total Op. Exoneradas S/</td> </b>';
    $totales .= ' <td width="110" align="right">' . number_format($tot_opexo, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

    $totales .= ' <tr>';
    $totales .= ' <b> <td width="140" height="18"align="right">Total '.$array_imp ['IVA'].' '. number_format($array_porc ['IVA'],2).'%:</td> </b>';
    $totales .= ' <td width="110" align="right">' . number_format($ncre_iva, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

  
    $totales .= ' <tr>';
    $totales .= ' <b> <td width="140" height="18" align="right">Total Descuentos S/</td> </b>';
    $totales .= ' <td width="110" align="right">' . number_format($totalDescuento, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

    $totales .= ' <tr>';
    $totales .= ' <b> <td width="140" height="18" align="right">Total ISC S/</td> </b>';
    $totales .= ' <td width="110" align="right">0.00</td>';
    $totales .= ' </tr>';


    $totales .= ' <tr>';
    $totales .= ' <b> <td width="140" height="18" align="right">Importe Total S/</td> </b>';
    $totales .= ' <td width="110" align="right">' . number_format($ncre_iva + $ncre_con_miva + $ncre_sin_iva, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';
    $totales .= ' </table>';
    ...
    file_put_contents('qr_nota_credito/NC_'.$id.'.png', $imageData); // Guardamos el resultado
