...
$sql = "SELECT empr_cod_prov, empr_cod_cant, empr_web_color, empr_cod_pais,empr_cm1_empr, empr_rimp_sn, empr_nom_empr, 
               empr_ruc_empr , empr_dir_empr, empr_conta_sn, empr_num_resu, empr_path_logo, empr_img_rep, 
               empr_iva_empr,empr_tel_resp, empr_ac1_empr, empr_ac2_empr, empr_mai_empr, empr_tip_empr
    ...
    if (!empty($empr_cod_prov)) {
        ...
        $prov_des_prov = consulta_string($sql, 'prov_des_prov', $oCon, 'NA');

    }

    if (!empty($empr_cod_cant)) {
        $sql = "SELECT cant_des_cant from saecant where cant_cod_cant = $empr_cod_cant ";
        $cant_des_cant = consulta_string($sql, 'cant_des_cant', $oCon, 'NA');
    }
    ...
    if ($oIfx->Query($sql)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            $sql = "SELECT clv_con_clpv from saeclpv where clpv_cod_clpv = $guia_cod_clpv";
    ...
    $sqlg="SELECT prov_des_prov, cant_des_cant, parr_des_parr
            from saedire 
            ...
            where trim(dire_dir_dire) like '$guia_dir_clie%' and dire_cod_clpv=$guia_cod_clpv and dire_cod_empr=$idEmpresa";

    $prov_clie=consulta_string($sqlg, 'prov_des_prov', $oIfxA, '');
    $cant_clie=consulta_string($sqlg, 'cant_des_cant', $oIfxA, '');
    $parr_clie=consulta_string($sqlg, 'parr_des_parr', $oIfxA, '');
    ...
    if($etiqueta=='1'){
        ...
        $tablePDF.='<td width="550" align="right"><br><b>PARA:</b><font style="color:#081f4b;"><br><br>'.$guia_nom_cliente.'<br><br>'.$guia_dir_clie.'<br><br>'.$prov_des_prov.' - '.$cant_des_cant.'<br><br>CEL. '.$guia_tlf_cliente.'</font>
    ...
    if($etiqueta=='2'){
        ...
        $tablePDF.='</tr>';
        $tablePDF.='<tr>';
        $tablePDF.='<td width="1010" align="center"><font style="color:#002254;"><b>'.$guia_nom_cliente.'<br><b>'.$tip_iden_cliente.':</b> '.$guia_ruc_clie.'&nbsp;&nbsp;&nbsp;<b>TELEFONO: '.$guia_tlf_cliente.'</b><br>
        '.$guia_dir_clie.'</b><br>'.$prov_clie.'-'.$cant_clie.'-'.$parr_clie.'</font><br><br><font style="color:red;font-size:26px;"><b>ENTREGA A '.$tipo.'</b></font><br><font style="color:#081f4b;">'.$atencion_oficina.'</font></td>';
        $tablePDF.='</tr>';
        $tablePDF.='<tr>';
        ...
        $tablePDF.='<tr>';
        $tablePDF.='<td width="1010" align="center"><font style="color:#002254;"><b>'.$razonSocial.'</b><br><br>'.$prov_des_prov.' - '.$cant_des_cant.'</font></td>';
        $tablePDF.='</tr>';
        $tablePDF.='</table>';
        $html2pdf = new HTML2PDF('L', 'A4', 'es');
    }
    ...
    if($etiqueta=='3'){
        ...
        $tablePDF.='<tr>';
        $tablePDF.='<td width="400" align="center"><font style="color:#002254;"><b>'.$razonSocial.'</b><br><br>'.$prov_des_prov.' - '.$cant_des_cant.'</font></td>';
        $tablePDF.='</tr>';
        $tablePDF.='</table>';
