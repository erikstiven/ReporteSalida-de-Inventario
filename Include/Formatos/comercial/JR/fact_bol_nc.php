...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
  ...
  $arrayTipDoc = array_dato($oCon, $sql, 'clpv_cod_clpv', 'clv_con_clpv');
    ...
    //  AMBIENTE - EMISION
    $sql = "select sucu_tip_ambi, sucu_tip_emis, sucu_telf_secu, sucu_dir_sucu  from saesucu where sucu_cod_empr = $idEmpresa and sucu_cod_sucu = $idSucursal ";
    if ($oIfx->Query($sql)) {
        if ($oIfx->NumFilas() > 0) {
    ...
    if ($oIfx->Query($sqlFac)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $moneda_local = consulta_string($sqlmon, 'pcon_mon_base', $oCon, '');
                ...
                //ETIQUETA LOCAL
                $sql = "select mone_des_mone, mone_sgl_mone, mone_smb_mene from saemone where mone_cod_mone =  $moneda_local;";
                ...
                $logo .= '<td width="335" style="font-size:15px;"><div style="margin-left:10px"><b>' . $razonSocial . '</b><br>'.$sucu_dir_sucu.'</div></td>';
                $logo .= '</tr>';
                $logo.='</table>';
                $logo .= '</td>';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $deta .= ' <td style="width: 55%;" align="justify">' . $dfac_nom_prod . '&nbsp;&nbsp;'.$dfac_det_dfac.'</td>';
            ...
            $deta .= ' <tr>';
            $deta .= ' <td style="width: 55%;">' . $fact_cm1_fact . '</td>';
    ...
    if($tipo_pdf=='F'){
        ...
        <br>
        Porcentaje de detracción: <b>'.round($fact_porc_detra,2).'%</b>
        </td>';
        $totales .= '</tr>';
    ...
    and emifa_tip_doc = 'FAC' and emifa_alia_emifa like '%$doc%' and emifa_est_emifa = 'S' 
