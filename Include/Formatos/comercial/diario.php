...
function formato_diario($idempresa = "", $idsucursal = "", $asto_cod = "", $ejer_cod = "", $prdo_cod = "")
{
    ...
    /// dir
    $sql = "select dir_num_fact from saedir where dire_cod_asto='$asto_cod_asto' and dire_cod_empr='$idempresa' and dire_cod_sucu='$idsucursal' and 
				asto_cod_ejer='$ejer_cod' and asto_num_prdo='$prdo_cod'";
    $dir_num_fact = consulta_string_func($sql, 'dir_num_fact', $oIfx, '');

    /// ret
    $sql = "select ret_num_ret from saeret where rete_cod_asto='$asto_cod_asto' and asto_cod_empr='$idempresa' and asto_cod_sucu='$idsucursal' and 
				asto_cod_ejer='$ejer_cod' and asto_num_prdo='$prdo_cod'";
    $dir_num_ret = consulta_string_func($sql, 'ret_num_ret', $oIfx, '');

    $rutaImagen = $empr_path_logo;

    $html .= '
        <!-- <table border = "0" style="width: 80%; margin-left: 50px; margin-top:20px">
            <tr>
                <td style="width:50%; font-size:18px; text-align: left;">' . $empr_logo . '</td>
            </tr>
        </table> --> 
        <table border="0" style="width: 72%; margin-left: 50px;">
            <tr>
                <td style="width: 50%; font-size:24px; font-weight: bold; text-align: left">' . $empr_nom . '</td>
                <td style="width:50%; font-size:24px; text-align: center"> Comprobante de Diario</td>  
            </tr>
            <tr>
                <td style="width: 80%; font-size:16px; text-align: left"><strong>DIRECCION:' . $empr_dir . '</strong></td>
                <td style="width: 20%; font-size:24px; text-align: center"><strong>&nbsp;&nbsp;D-' . $asto_cod_asto . '</strong></td>
            </tr>
		</table>
        <!-- <td style="width: 50%; font-size:16px; text-align: left"><strong>Modulo:</strong>' . $modu_des_modu . '</td> -->
        <table border="0" style="width: 68%; margin-left: 50px;">
				<tr>
					<td style="width: 70%; font-size:16px; text-align: left"><strong>Fecha:</strong>' . $ciudad . ', ' . $asto_fec . '</td>
                    <td style="width: 40%; font-size:22px; text-align: left"><strong>Monto: ' . number_format($asto_vat_asto, 2, '.', ',') . '</strong></td>
				</tr>
				<tr>
					<td style="width: 50%; font-size:16px; text-align: left"><strong>Documento:</strong> ' . $asto_cod_tidu . ' - ' . $tidu_des_tidu . '</td>
				</tr>
				<tr>
					<td  style="width: 100%; font-size:16px; text-align: left"><strong>Beneficiario: </strong> ' . $asto_ben_asto . '</td>
				</tr>
			
				<tr>
					<td style="width: 80%; font-size:16px; text-align: left"><strong>Detalle:</strong> ' . $asto_det_asto . '</td>
                    <td style="width: 20%; font-size:16px; text-align: left"><strong>Retencion Nro:</strong> ' . $dir_num_ret . '</td>
				</tr>
				<tr>
					<td colspan="2" style="width: 50%; font-size:16px; text-align: left"><strong>Factura Nro:</strong> ' . $dir_num_fact . '</td>
				</tr>';
...
/*
    if (!empty($arrayDirectorio)) 
    {
        if (count($arrayDirectorio) > 0) {
            ...
            foreach ($arrayDirectorio as $val) {
                ...
                $dir_deb_mex = $val[9];
                $dir_cre_mex = $val[10];
                $dire_cod_asto = $val[11];
                $dire_cod_empr = $val[12];
                $dire_cod_sucu = $val[13];
                $asto_cod_ejer = $val[14];
                $asto_num_prdo = $val[15];

                $sql_asto_mone = "SELECT 
                                    asto_cod_mone 
                                from 
                                    saeasto 
                                where 
                                    asto_cod_asto = '$dire_cod_asto'
                                    and asto_cod_empr = '$dire_cod_empr'
                                    and asto_cod_sucu = $dire_cod_sucu
                                    and asto_cod_ejer = $asto_cod_ejer
                                    and asto_num_prdo = $asto_num_prdo
                                ";
                $asto_cod_mone = consulta_string_func($sql_asto_mone, 'asto_cod_mone', $oIfx, 0);

                $sql_mone_base = "SELECT pcon_mon_base from saepcon where pcon_cod_empr = $idempresa";
                $mone_empr_base = consulta_string_func($sql_mone_base, 'pcon_mon_base', $oIfx, 0);

                
                //if ($asto_cod_mone != $mone_empr_base) {
                //    $dir_deb_ad = $dir_deb_mex;
                //    $dir_cre_ad = $dir_cre_mex;
                //} else {
                //    $dir_deb_ad = $dir_deb_ml;
                //    $dir_cre_ad = $dir_cre_ml;
                //}
                ...
                $html .= '<td style="width:10%;font-size:12px; text-align: right;  border-right:1px solid; border-top:1px solid" align="right">' . number_format($dir_deb_ad, 2, '.', ',') . '</td>';
                $html .= '<td style="width:10%;font-size:12px; text-align: right; border-top:1px solid" align="right">' . number_format($dir_cre_ad, 2, '.', ',') . '</td>';
                $html .= '</tr>';
            }
            $html .= '</table>';
        }
    ...
    ///diario
    if (!empty($arrayDiario)) {
        if (count($arrayDiario) > 0) {

            $html .= '<table style="margin-left:50px; width:90%; border:0px ; border-radius: 0px; margin-top:20px" align="left">';
            $html .= '<tr>';
            $html .= '<td colspan="4" style="width:100%;font-size:18px; text-align: center; border-bottom:1px;"><strong>DIARIO</strong></td>';
            $html .= '</tr>';
            $html .= '<tr>';
            $html .= '<td style="width:15%;font-size:16px; text-align: center;  border-bottom:1px; border-right:1px "><strong>Codigo</strong></td>';
            $html .= '<td style="width:65%;font-size:16px; text-align: center;  border-bottom:1px; border-right:1px "><strong>Descripcion/Detalle</strong></td>';
            $html .= '<td style="width:10%;font-size:16px; text-align: center;  border-bottom:1px; border-right:1px "><strong>Debito</strong></td>';
            $html .= '<td style="width:10%;font-size:16px; text-align: center;  border-bottom:1px; border-right:0px "><strong>Credito</strong></td>';
            $html .= '</tr>';
            $totalDeb = 0;
            $totalCre = 0;
            ...
            foreach ($arrayDiario as $val) {
                ...
                if ($dasi_cod_cuen=='2.1.01.03.001' && $asto_cod_modu == 5)
                {
                    $html .= '<tr>';
                    $html .= '<td style="width:15%;font-size:16px; text-align: center;  border-bottom:0px; border-right:0px ">' . $dasi_cod_cuen .  '</td>';
                    $html .= '<td style="width:65%;font-size:16px; text-align: left;  border-bottom:0px; border-right:0px ">' . $cuen_nom_cuen . ' - ' . substr($dasi_det_asi, 0, 40) .  '</td>';
                    $html .= '<td style="padding: 8px; width:10%;font-size:16px; text-align: right; border-bottom:0px; border-right:0px; margin-top:20px "><label >' . number_format($dasi_dml_dasi, 2, '.', ',') . '</label></td>';
                    $html .= '<td style="padding: 8px; width:10%;font-size:16px; text-align: right; border-bottom:0px; border-right:0px ">' . number_format($dasi_cml_dasi, 2, '.', ',') . '</td>';
                    $html .= '</tr>';
                }else
                {
                    $html .= '<tr>';
                    $html .= '<td style="width:15%;font-size:16px; text-align: center;  border-bottom:0px; border-right:0px ">' . $dasi_cod_cuen .  '</td>';
                    $html .= '<td style="width:65%;font-size:16px; text-align: left;  border-bottom:0px; border-right:0px ">' . $cuen_nom_cuen . '</td>';
                    $html .= '<td style="padding: 8px; width:10%;font-size:16px; text-align: right; border-bottom:0px; border-right:0px; margin-top:20px "><label >' . number_format($dasi_dml_dasi, 2, '.', ',') . '</label></td>';
                    $html .= '<td style="padding: 8px; width:10%;font-size:16px; text-align: right; border-bottom:0px; border-right:0px ">' . number_format($dasi_cml_dasi, 2, '.', ',') . '</td>';
                    $html .= '</tr>';
                }
            ...
            $html .= '<tr>';
            $html .= '<td align="right" style="width:60%;font-size:16px; text-align: right; border-bottom:1px; border-top:1px;" colspan="2"><strong>TOTAL</strong></td>';
            $html .= '<td align="right" style="padding: 8px; width:8%;font-size:16px; text-align: right; border-bottom:1px; border-top:1px;"><strong>' . number_format($totalDeb, 2, '.', ',') . '</strong></td>';
            $html .= '<td align="right" style="padding: 8px; width:8%;font-size:16px; border-bottom:1px; border-top:1px;"><strong>' . number_format($totalCre, 2, '.', ',') . '</strong></td>';
            $html .= '</tr>';
            $html .= '</table>';
            $sql = "select minv_num_comp, minv_num_caja, minv_prec_caja from saeminv where minv_cod_empr='$idempresa' and minv_cod_sucu='$idsucursal' and 
				minv_cod_ejer='$ejer_cod' and minv_num_prdo='$prdo_cod' and minv_tran_minv='$asto_cod'";
            ...
            <tr>
                ...
                <td style="width:15%; font-size:14px; text-align: center;border-top : 2px solid black;">Elaborado por:<br>' . $usuario . '</td>
                <td style="width:10%;"></td>
                <td style="width:15%;font-size:14px; text-align: center;border-top : 2px solid black;">Revisado por</td>
                <td style="width:10%;"></td>	
                <td style="width:15%; font-size:14px; text-align: center;border-top : 2px solid black;">Aprobado por:</td>
                <td style="width:10%;"></td>
                <!-- <td style="width:10%; font-size:14px; text-align: center;border-top : 2px solid black;">Recibí Conforme</td>
                <td style="width:10%;"></td> -->
            </tr>
