...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
 ...
 $empr_cod_pais = round(consulta_string($sql, 'empr_cod_pais', $oIfx, 0));
  ...
  $arrayTipDoc = array_dato($oCon, $sql, 'clpv_cod_clpv', 'clv_con_clpv');
        ...
        if ($oIfx->NumFilas() > 0) {
            do {
               ...
               // $sql = "select tcmp_cod_tcmp, tcmp_des_tcmp from saetcmp where tcmp_cod_tcmp = '$fact_tip_vent';";
                ...
                $logo .= '<tr style="font-size:16px;">';
                $logo .= '<td width="260"  height="35" style="background: '.$empr_web_color.'; color:white;" align="center"><b>FACTURA ELECTRÓNICA</b></td>';
                $logo .= '</tr>';
    ...
    $deta .= ' <tr>';
    $deta .= ' <b> <td style="border-top-left-radius: 4px; background: '.$empr_web_color.'; color:white; width: 53%; font-size:13px;" align="center" height="30">DESCRIPCIÓN</td> </b>';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $deta .= ' <tr>';
                $deta .= ' <td style="width: 53%;">'.$dfac_det_dfac.'</td>';
                $deta .= ' <td style="width: 5%;" align="center" style="border-left: '.$empr_web_color.' 0.7px solid;">' . $unidad . '</td>';
                $deta .= ' <td style="width: 10%;" align="right" style="border-left: '.$empr_web_color.' 0.7px solid;">' . number_format($dfac_cant_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 11%;" align="right" style="border-left: '.$empr_web_color.' 0.7px solid;">' . number_format($dfac_precio_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 9%;" align="right" style="border-left: '.$empr_web_color.' 0.7px solid;">' . number_format($porcentaje_descuento, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 12%;" align="right" style="border-left: '.$empr_web_color.' 0.7px solid;">' . number_format($dfac_mont_total, 2, '.', ',') . '</td>';
                $deta .= ' </tr>';
                $ctrl++;
            }while ($oIfx->SiguienteRegistro());
             $deta .= ' <tr>';
    ...
    if(!empty($fact_cod_detra)){
        ...
        if(round($fact_tot_fact,2)>round($tret_mon_min,2)){
           ...
           /* $totales .='<tr>
            <td width="390"><b>Detracción al '.round($fact_porc_detra,2).'%:</b> '.$sigmoneprin.' '.round($valdetra,0,PHP_ROUND_HALF_UP).' </td>
            </tr>
            <tr>
    ...
    /*if($tipo_pdf=='F'&&$idEmpresa==1){
        ...
        Código BB Y SS sujetos a DETRACCIÓN: <b>019 y 037</b><br>
    ...
    if($fact_cod_mone==$pcon_seg_mone)
    {
        $fact_tot_fact_local=$fact_tot_fact;
    ...
    {
        $totales .= ' <tr>';
        $totales .= ' <b> <td width="185" height="18" align="right">Total '.$sigmoneprin.'</td> </b>';    
        $totales .= ' <td width="110" align="right">' . number_format($fact_tot_fact_local, 2, '.', ',') . '</td>';
        $totales .= ' </tr>';
    }
    $totales .= ' </table>';
    ...
    $tableLeyenda .= '<td width="500" height="88"><div style="margin-left:5px;margin-top:10px;">Representación impresa de la: <b>FACTURA ELECTRÓNICA</b><br>
    </div></td>';
    $tableLeyenda .= '</tr>';
