...
function reporte_factura_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
 ...
 $empr_cod_pais = round(consulta_string($sql, 'empr_cod_pais', $oIfx, 0));
  ...
  $arrayTipDoc = array_dato($oCon, $sql, 'clpv_cod_clpv', 'clv_con_clpv');
    ...
    if ($oIfx->Query($sqlFac)) {
        if ($oIfx->NumFilas() > 0) {
            do {
                ...
                $fact_dsg_porc  = $oIfx->f('fact_dsg_porc');
                ...
                //query forma de pago
        $sqlFPago = "select  fx.fxfp_cot_fpag
				from saefact f, saefxfp fx
				where 
                ...
                $metodo_pago= consulta_string($sql,'fxfp_cot_fpag', $oCon,'');
                if ($metodo_pago == 'EFE') {
                    $tipo_pago = 'EFECTIVO';
                } else if ($metodo_pago == 'CRE') {
                    $tipo_pago = 'CREDITO';
                } else {
                    $tipo_pago = 'CONTADO';
                }
                ...
                $logo .= '<tr>';
                $logo .= '<td align="center" width="50">'.$logo_empresa.'</td>';
                $logo .= '<td align="center" style="font-size:11px;" width="150"><b>' . $razonSocial . '</b><font size="8"><br>RIF: '.$ruc_empr.' '.$dirMatriz.' Telefonos: '.$tel_empresa.'<br> E -mail: '.$empr_mai_empr.'</font></td>';
                $logo .= '</tr>';
                $logo.='</table>';
                $logo .= '</td>';
                
                $logo .= '<td >';
                $logo .= '<table  style="margin: 0px;" cellspacing=0>';
            
                $logo .= '<tr style="font-size:11px;">';
                $logo .= '<td width="100" height="35" align="center"></td>';
                $logo .= '</tr>';
            
                $logo .= '<tr style="font-size:14px;">';
                $logo .= '<td width="100"  height="35" style=" color:red;" align="center"><b>'.$fact_num_preimp.'</b></td>';
                $logo .= '</tr>';
                
                $logo .= '<tr style="font-size:14px;">';
                $logo .= '<td width="100" height="35"  style=" color:blue;" align="center" ><b>N° DE CONTROL</b><br>FORMA LIBRE</td>';
                $logo .= '</tr>';
                $logo.='</table>';
                $logo .= '</td>';
                ...
                $logo ='<table border="0" style="width: 100%; font-size:12px;">';
                $logo .= '<tr>';
                $logo .= '<td width="230">&nbsp;</td>';
                $logo .= '<td width="120" align="right"><b>SERIE.:</b></td>';
                $logo .= '<td width="110" ><b>'.$nse_fact.'</b></td>';
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="230"> FECHA EMISION: '.$fact_fech_fact.' </td>';
                $logo .= '<td  width="120" align="right"><b>FACTURA No.:</b></td>';
                $logo .= '<td  width="110" >'.$fact_num_preimp.'</td>';
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="230" >NOMBRE/RAZON SOCIAL.</td>';  //VALIDAR
                $logo .= '<td  width="120" align="right">N° CEDULA/RIF:</td>';
                $logo .= '<td  width="110" >'.$fact_ruc_clie.'</td>';
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="460"  colspan="3">'.$fact_nom_cliente.'</td>';
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="460"  colspan="3">TELEFONO: '.$fact_tlf_cliente.'</td>';
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="230" >DOMICILIO FISCAL</td>';  //VALIDAR
                $logo .= '<td  width="120" align="right">METODO DE PAGO:</td>';
                $logo .= '<td  width="110" >'.$tipo_pago.'</td>';
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="460"  colspan="3">'.$fact_dir_clie.'</td>';
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="460"  colspan="3">CIUDAD: </td>';//VALIDAR
                $logo .= '</tr>';

                $logo .= '<tr>';
                $logo .= '<td  width="460"  colspan="3">MUNICIPIO.: </td>';//VALIDAR
                $logo .= '</tr>';
    ...
    $deta .= ' <table style="width: 96%;  font-size: 12px; margin-top:10px;margin-left:2px;" cellspacing="0"; border="0"; >';
    $deta .= ' <tr>';
    $deta .= ' <b> <td style="border-bottom: 1px solid; width: 10%; font-size:12px;" align="left" height="30">CAN</td> </b>';
    $deta .= ' <b> <td style="border-bottom: 1px solid; width: 60%; font-size:12px;" align="center" height="30">DESCRIPCION</td> </b>';
    $deta .= ' <b> <td style="border-bottom: 1px solid; width: 15%; font-size:12px;" align="center" height="30">P.UNITARIO</td> </b>';
    $deta .= ' <b> <td style="border-bottom: 1px solid; width: 15%; font-size:12px;" align="center" height="30">MONTO</td> </b>';
    $deta .= ' </tr>';
    ...
    if ($oIfx->Query($sqlDeta)) {
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                $deta .= ' <tr>';
                $deta .= ' <td style="width: 10%;" align="right">' . number_format($dfac_cant_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 60%;">'.$dfac_det_dfac.'</td>';
                $deta .= ' <td style="width: 15%;" align="right" >' . number_format($dfac_precio_dfac, 2, '.', ',') . '</td>';
                $deta .= ' <td style="width: 15%;" align="right" >' . number_format($dfac_mont_total, 2, '.', ',') . '</td>';
                $deta .= ' </tr>';
                $ctrl++;
            }while ($oIfx->SiguienteRegistro());
            $deta .= ' <tr>';
            $deta .= ' <td style="border-top: 1px dashed; width: 10%;" align="right"></td>';
            $deta .= ' <td style="border-top: 1px dashed; width: 60%;"></td>';
            $deta .= ' <td style="border-top: 1px dashed; width: 15%;" align="right" ></td>';
            $deta .= ' <td style="border-top: 1px dashed; width: 15%;" align="right" ></td>';
            $deta .= ' </tr>';
            $deta .= ' <tr>';
            $deta .= ' <td style="border-top: 1px dashed; width: 10%;" align="right"></td>';
    ...
    $tablepago.='<table  style="width:96%" border="0" cellspacing="5">';
    $tablepago .= '<tr>';
    ...
    $tablepago .= '<td valign="top" width="100">';
        ...
        if ($oIfx->NumFilas() > 0) {
            ...
            do {
                ...
                if(!empty($fact_cod_detra)){
                    ...
                    $fact_porc_detra=consulta_string($sql,'tret_porct',$oCon,0);
                    $tret_mon_min=consulta_string($sql,'tret_mont_min',$oCon,0);

                    if(round($fxfp_val_fxfp,2)>round($tret_mon_min,2)){
                        ...
                        $fact_tot_detra = $fxfp_val_fxfp;
                        
                       
                        //VALOR NETO EN MOENDA EXTRANJERA
                        $valdetra_cam=$fact_tot_detra* ($fact_porc_detra/100);
                        $valor_neto=$fact_tot_detra-round($valdetra_cam,0,PHP_ROUND_HALF_UP);

                        $fxfp_val_fxfp=$valor_neto;
    ...
    $totales ='<table style="width: 96%;  font-size: 11px; margin-top: 5px;" cellspacing="0"  align="left">';
    $totales .='<tr> ';
    $totales .= '<td style="width: 85%;" height="18" align="right">SUB TOTAL '.$eti_mone.'.:</td>';
    $totales .= '<td  style="width: 15%;" align="right">' . number_format($fact_con_miva+$fact_sin_miva + $fact_val_irbp -$fact_dsg_valo, 2, '.', ',') . '</td>';
    $totales .= '</tr>';

    $totales .= ' <tr>';
    $totales .= ' <td style="width: 85%;" height="18" align="right">DESCUENTO SOBRE MENSUALIDAD ('. number_format($fact_dsg_porc,2).'%) '.$eti_mone.'.:</td>';
    $totales .= ' <td style="width: 15%;"  align="right">' . number_format($totalDescuento, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

    $totales .= ' <tr>';
    $totales .= ' <td style="width: 85%;" height="18" align="right">BASE IMPONIBLE ('. number_format($array_porc ['IVA'],2).'%)  '.$eti_mone.'.:</td>';
    $totales .= ' <td style="width: 15%;" align="right">' . number_format($fact_con_miva, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

    $totales .= ' <tr>';
    $totales .= ' <td style="width: 85%;" height="18" align="right">IVA ('. number_format($array_porc ['IVA'],2).'%)  '.$eti_mone.'.:</td>';
    $totales .= ' <td style="width: 15%;" align="right">' . number_format($fact_iva, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';

    $totales .= ' <tr>';
    $totales .= ' <td style="width: 85%;" height="18" align="right">TOTAL A PAGAR '.$eti_mone.'</td>';
    $totales .= ' <td style="width: 15%;" align="right">' . number_format($fact_tot_fact, 2, '.', ',') . '</td>';
    $totales .= ' </tr>';
    $totales .= ' </table>';
    ...
    $ruta_xml = 'modulos/envio_documentos_venezuela/upload/xml/fac_' . $nombre_documento . '.xml';
    ...
    Recuerda enviar tu comprobante de pago al: <b>+51 931 388 888</b><br>
    clientesmasivos@globalfiber.com.pe
    <br>
    ...
    $ruta_dir = DIR_FACTELEC . 'modulos/envio_documentos_venezuela/qr_facturas';
        if (!file_exists($ruta_dir)){
            mkdir($ruta_dir,0777,true);
        }
        
    file_put_contents(DIR_FACTELEC . 'modulos/envio_documentos_venezuela/qr_facturas/FAC_'.$id.'.png', $imageData); // Guardamos el resultado
    
    $ruta=DIR_FACTELEC . 'modulos/envio_documentos_venezuela/qr_facturas/FAC_'.$id.'.png';
    ...
    $tabla = '<page backimgw="100%" backtop="5mm" backbottom="5mm" backleft="5mm" backright="5mm">';

    $tabla .= '<table style="width: 100%; margin-top: 10px;" border="0";>
    <tr>
    <td style="width: 45%;">' . $documento . '</td>
    ...
    $ruta_dir= DIR_FACTELEC . 'modulos/envio_documentos_venezuela/upload';
	if (!file_exists($ruta_dir)){
					mkdir($ruta_dir,0777,true);
	}
    $ruta = DIR_FACTELEC . 'modulos/envio_documentos_venezuela/upload/pdf/fac_' . $nombre_archivo . '.pdf';
...
function reporte_notaCredito_personalizado($id = '', $nombre_archivo = '', $idSucursal ='', &$rutaPdf = '') {
 ...
 $empr_cod_pais = round(consulta_string($sql, 'empr_cod_pais', $oIfx, 0));
    ...
    $ruta_dir = DIR_FACTELEC . 'modulos/envio_documentos_venezuela/qr_nota_credito';
        if (!file_exists($ruta_dir)){
            mkdir($ruta_dir,0777,true);
        }

    file_put_contents(DIR_FACTELEC . 'modulos/envio_documentos_venezuela/qr_nota_credito/NC_'.$id.'.png', $imageData); // Guardamos el resultado
    
    $ruta=DIR_FACTELEC . 'modulos/envio_documentos_venezuela/qr_nota_credito/NC_'.$id.'.png';*/

    $tableLeyenda .= '<tr>';
    $tableLeyenda .= '<td width="204" align="center"></td>';
    ...
    $ruta = DIR_FACTELEC . 'modulos/envio_documentos_venezuela/upload/pdf/cred_' . $nombre_archivo . '.pdf';
    $html2pdf->Output($ruta, 'F');
    $rutaPdf = $ruta;

    $documento='FORMATO NO CONFIGURADO';
