...
class RouterosAPI
{
    ...
    public function debug($text)
    {
        ...
        if (strpos($text, 'message') !== false) {
            throw new Exception(substr($text, 20, strlen($text)));
        }
    }
    /**
     *
    ...
    /**
     * Login to RouterOS new method 6.45.1 and above
     *
    ...
    /**
     * Login to RouterOS old method 6.44 and under
     *
     * @param string      $ip         Hostname (IP or domain) of the RouterOS server
     * @param string      $login      The RouterOS username
     * @param string      $password   The RouterOS password
     ...
     * @return boolean                If we are connected or not
     */
    public function connect_old($ip, $login, $password)
    {
        for ($ATTEMPT = 1; $ATTEMPT <= $this->attempts; $ATTEMPT++) {
            $this->connected = false;
            $PROTOCOL = ($this->ssl ? 'ssl://' : '' );
            ...
            foreach ($response as $x) {
                if (in_array($x, array('!fatal','!re','!trap'))) {
                    ...
                    if (preg_match_all('/[^=]+/i', $x, $MATCHES)) {
                        ...
                        $CURRENT_DATE[$MATCHES[0][0]] = (isset($MATCHES[0][1]) ? $MATCHES[0][1] : '');
                    }
                }
            }
