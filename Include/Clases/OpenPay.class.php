...
class OpenPayAPI
{
    ...
    {
        // informacion de la api de openpay
        $oConexion1 = $this->oConexion1;
        $empr_openpay_sn = '';
        $empr_openpay_url = '';
        $empr_openpay_pass = '';
        $empr_openpay_user = '';
        $empr_openpay_api_token = '';

        $sql_empr = "SELECT
                        empr_openpay_sn,
                        empr_openpay_url, 
                        empr_openpay_api_version, 
                        empr_openpay_merchantid, 
                        empr_openpay_private_key, 
                        empr_openpay_public_key 
                    FROM saeempr 
        ...
        if ($oConexion1->Query($sql_empr)) {
            if ($oConexion1->NumFilas() > 0) {
                do {
                    ...
                    $empr_openpay_sn                = trim($oConexion1->f('empr_openpay_sn'));
                    $empr_openpay_url               = trim($oConexion1->f('empr_openpay_url'));
                    $empr_openpay_api_version       = trim($oConexion1->f('empr_openpay_api_version'));
                    $empr_openpay_api_environment   = trim($oConexion1->f('empr_openpay_api_environment'));
                    $empr_openpay_merchantid        = trim($oConexion1->f('empr_openpay_merchantid'));
                    $empr_openpay_private_key       = trim($oConexion1->f('empr_openpay_private_key'));
                    $empr_openpay_public_key        = trim($oConexion1->f('empr_openpay_public_key'));
        ...
        if ($empr_openpay_sn == 'S' && !empty($empr_openpay_url) && !empty($empr_openpay_merchantid) && !empty($empr_openpay_private_key) && !empty($empr_openpay_public_key)) {
            $info = array(
                "base_url"          => "$empr_openpay_url",
                "username"          => "$empr_openpay_user",
                "password"          => "$empr_openpay_pass",
                "version"           => "$empr_openpay_api_version",
                "environment"       => "$empr_openpay_api_environment",
                "test_urlexample"   => "https://sandbox-api.openpay.pe",
                "endpoints" => array(
                    "save_customer" => array(
                        "path"              => "$empr_openpay_api_version/{{merchantid}}/customers",
                        "descripcion"       => "Endpoint para registrar clientes",
                        "method"            => "POST",
                        "param"             => false,
                        "body"              => true,
                        "auth_type"         => "Basic",
                        "Username"          => "$empr_openpay_private_key",
                        "Password"          => "",
                    ...
                    "save_card" => array(
                        "path"              => "$empr_openpay_api_version/{{merchantid}}/customers/{{customerid}}/cards",
                        "descripcion"       => "Endpoint para registrar tarjetas",
                        "method"            => "POST",
                        "param"             => false,
                        "body"              => true,
                        "auth_type"         => "Basic",
                    ...
                    "save_bill" => array(
                        "path"              => "$empr_openpay_api_version/{{merchantid}}/customers/{{customerid}}/charges",
                        "descripcion"       => "Endpoint para registrar un cargo al cliente",
                        "method"            => "POST",
                        "param"             => false,
                        "body"              => true,
                        "auth_type"         => "Basic",
    ...
    function api_request($url, $body, $token, $method, $user, $password, $auth_type)
    {
        ...
        switch($auth_type){
            case "Basic":
                ...
                if (!empty($user) && !empty($password)) {
                    // array_push($header_array, "Authorization: $auth_type $user:$password");
                    curl_setopt($curl, CURLOPT_USERPWD , "$user:$password");

                }else{
                    throw new Exception("Error determinar credenciales $auth_type", 1);                    
                }
            ...
            case "Bearer":
                if (!empty($token)) {
                    array_push($header_array, "Authorization: $auth_type $token");
                }else{
                    throw new Exception("Error determinar credenciales $auth_type", 1);                    
                }
            break;
