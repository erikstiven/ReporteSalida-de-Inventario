...
if ($accion == 'subir') {
	...
	if ($ctr) { ?>
		<script>
			...
			if (window.opener && window.opener.document.form1) {
				if (window.opener.document.form1['<?= $_REQUEST['control'] ?>']) {
			...
			if (window.opener && window.opener.document.formpago) {
				if (window.opener.document.formpago['<?= $_REQUEST['control'] ?>']) {
					// El campo existe en el documento del padre
					//console.log('El campo existe en el documento del padre.');
